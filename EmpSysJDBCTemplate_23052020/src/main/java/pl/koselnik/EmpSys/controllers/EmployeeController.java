...
//        addresses.addAll(addressService.getAddresses());
//        System.out.println(addresses);

        model.addAttribute("employee", new Employee()).addAttribute("addresses", addressService);
    ...
    public String saveEmployeeAndAddress(@Valid @ModelAttribute("employee")  Employee employee,
                                        BindingResult bindingResultEmployee,
                                         @Valid @ModelAttribute ("addresses") AddressCreationDto addresses,
                                        BindingResult bindingResultAddress, Model model)  {

        System.out.println("Są błędy");
        System.out.println(addresses);
        if(bindingResultEmployee.hasErrors() || bindingResultAddress.hasErrors()) {

            bindingResultEmployee.getAllErrors().forEach(error -> {
                System.out.println(error.getObjectName() + " " + error.getDefaultMessage());
                }
            );
            bindingResultAddress.getAllErrors().forEach(error -> {
                        System.out.println(error.getObjectName() + " " + error.getDefaultMessage());
                    }
            );

            model.addAttribute("employee", employee).addAttribute("addresses", addresses);
            return "new_employee_form";
...
//            AddressRepository addressesList = addressesFromForm.getAddresses();
            Runnable runnableAddress = () -> addressService.saveAddressToDB(addresses.getAddresses());
//
